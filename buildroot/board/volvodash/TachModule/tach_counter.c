...
#define PULSE_COUNTER_GPIO_USE_DEBOUNCE 	0
#define PULSE_COUNTER_GPIO_DEBOUNCE_TIME_MS 100
...
static irqreturn_t pulse_irq_handler(__u32 irq, void * dev_id, struct pt_regs * regs){
...
#if PULSE_COUNTER_GPIO_USE_DEBOUNCE == 1
	...
	if((jiffies-lastInterrupt) > msecs_to_jiffies(PULSE_COUNTER_GPIO_DEBOUNCE_TIME_MS)){	
		lastInterrupt = jiffies;
		pulse_count_total++;
	}
#else
